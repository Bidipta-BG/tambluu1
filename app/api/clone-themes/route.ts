...
export async function GET() {
  try {
    ...
    generateTheme('ColorSplashDashboard.tsx', 'ColorSplashDashboard', 'Color Splash', [
      ...
      { search: escapeRegExp('bg-[#0a2617] border border-[#143a24]'), replace: 'bg-white border border-pink-200 text-purple-800' }, // Search bar
      ...
      { search: escapeRegExp('bg-[#0c2e1c] font-sans text-white'), replace: 'bg-white font-sans text-purple-800' }, // fallback bg
    ]);
    ...
    return NextResponse.json({ success: true, message: 'Themes cloned and styled successfully' });
  } catch (err: any) {
