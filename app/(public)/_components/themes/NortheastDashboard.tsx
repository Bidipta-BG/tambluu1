  ...
  return (
    ...
    >
      ...
      <div className="relative w-full pt-10 pb-6 px-4 flex flex-col items-center">
        ...
        {/* Title */}
        <h1 className="relative z-10 whitespace-nowrap text-2xl sm:text-6xl font-serif font-black text-[#f1e5c3] tracking-wider text-center drop-shadow-md uppercase">
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 py-2 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-1.5 sm:gap-4 py-2 w-full sm:flex sm:justify-center sm:items-center sm:w-auto">
          {/* Call Icon */}
          <a href={`tel:${tenant.whatsappNumber || ''}`} className="bg-[#143a24] hover:bg-[#1e4e31] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-[#205234] text-yellow-500 shadow-md transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto">
          ...
          {/* WhatsApp Icon */}
          <a href={buildWhatsAppUrl(tenant.whatsappNumber || '', 'Hi, I want to inquire about the Tambola game.')} target="_blank" rel="noopener noreferrer" className="bg-[#143a24] hover:bg-[#1e4e31] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-[#205234] text-[#25D366] shadow-md transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto">
          ...
          */}
          ...
          <div className="relative w-full sm:w-auto">
            <button 
              ...
              className="bg-[#143a24] hover:bg-[#1e4e31] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-[#205234] text-yellow-500 shadow-md transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto w-full"
              title="View Agents"
            >
            ...
            {showAgentsMenu && (
              <>
                ...
                <div className="absolute top-full right-0 sm:-right-4 mt-2 w-56 bg-[#0c2e1c] border border-[#205234] rounded-lg shadow-xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                  <div className="px-4 py-3 border-b border-[#205234] bg-[#0f3822]">
                    <h3 className="text-xs font-bold text-yellow-500 uppercase tracking-widest">Authorized Agents</h3>
                  </div>
                  <div className="max-h-60 overflow-y-auto">
                    {agents && agents.length > 0 ? (
                      agents.map(agent => (
                        <div key={agent.id} className="px-4 py-3 text-sm text-[#f0ecd8] border-b border-[#205234]/50 last:border-0 flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full bg-yellow-500"></div>
                          {agent.name}
                        </div>
                      ))
                    ) : (
                      <div className="px-4 py-4 text-xs text-[#a3b8ad] italic text-center">
                        No agents assigned
                      </div>
                    )}
                  </div>
        ...
        {!isLive && (
          <>
            ...
            <button
              ...
              className="w-full flex items-center justify-center mb-4 py-3.5 sm:py-4 rounded-xl bg-gradient-to-r from-[#163725] via-[#1e4e31] to-[#163725] border-2 border-yellow-500/50 text-yellow-400 shadow-[0_0_20px_rgba(234,179,8,0.3)] hover:shadow-[0_0_25px_rgba(234,179,8,0.5)] transition-all active:scale-[0.98]"
            >
              <span className="font-black text-sm sm:text-base tracking-widest uppercase drop-shadow-md">CHECK AVAILABLE TICKETS</span>
            </button>

        {/* STATS BAR */}
        <div className="flex flex-col rounded-xl bg-[#eef0e5] p-3 shadow-lg border-2 border-[#163725] divide-y-2 divide-[#163725]/20">
          {/* Row 1: Timer */}
          <div className="py-1 border-[#163725]/20">
            <CountdownTimer
              targetDate={displayGame.scheduled_at || new Date().toISOString()}
              variant="split"
              className="w-full text-[#163725] border-[#163725]/20"
          ...
          <div className="grid grid-cols-2 pt-3 pb-1">
            <div className="flex flex-col items-center justify-center border-r-2 border-[#163725]/20 px-2 text-center">
              <span className="text-sm sm:text-base font-bold text-[#163725]">{formattedDate}</span>
            </div>
            <div className="flex flex-col items-center justify-center px-2 text-center">
              <span className="text-sm sm:text-base font-black text-[#163725]">{formattedTime}</span>
            </div>
          </div>
          
          {/* Tickets Bought (Commented out as requested) 
          <div className="flex flex-col items-center justify-center pt-3 border-t-2 border-[#163725]/20 text-center">
