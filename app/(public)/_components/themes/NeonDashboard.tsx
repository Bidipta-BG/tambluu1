...
export default function NeonDashboard({
  ...
  return (
    <div 
      ...
      style={{ backgroundImage: "url('/images/neon_night.jpg')" }}
    >
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 py-2 space-y-4">
        ...
        <div className="grid grid-cols-4 gap-1.5 sm:gap-4 py-2 w-full sm:flex sm:justify-center sm:items-center sm:w-auto">
          {/* Call Icon */}
          <a href={`tel:${tenant.whatsappNumber || ''}`} className="bg-[#111] hover:bg-[#222] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-pink-500 text-cyan-400 shadow-[0_0_8px_rgba(236,72,153,0.5)] hover:shadow-[0_0_12px_rgba(236,72,153,0.8)] transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto">
          ...
          {/* WhatsApp Icon */}
          <a href={buildWhatsAppUrl(tenant.whatsappNumber || '', 'Hi, I want to inquire about the Tambola game.')} target="_blank" rel="noopener noreferrer" className="bg-[#111] hover:bg-[#222] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-pink-500 text-[#25D366] shadow-[0_0_8px_rgba(37,211,102,0.5)] hover:shadow-[0_0_12px_rgba(37,211,102,0.8)] transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto">
          ...
          <button 
            onClick={toggleSound}
            className="bg-[#111] hover:bg-[#222] p-3 rounded-full border border-pink-500 text-cyan-400 shadow-[0_0_8px_rgba(236,72,153,0.5)] hover:shadow-[0_0_12px_rgba(236,72,153,0.8)] transition-all flex items-center justify-center relative group"
          ...
          */}
          ...
          <div className="relative w-full sm:w-auto">
            <button 
              ...
              className="bg-[#1a1a2e] hover:bg-[#252542] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-pink-500/30 text-pink-500 drop-shadow-[0_0_5px_rgba(236,72,153,0.5)] transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto w-full"
              title="View Agents"
            >
            ...
            {showAgentsMenu && (
              <>
                ...
                <div className="absolute top-full right-0 sm:-right-4 mt-2 w-56 bg-[#1a1a2e] border border-pink-500/30 rounded-lg shadow-[0_0_15px_rgba(236,72,153,0.2)] overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                  <div className="px-4 py-3 border-b border-pink-500/30 bg-[#252542]">
                    <h3 className="text-xs font-bold text-pink-400 uppercase tracking-widest drop-shadow-[0_0_2px_rgba(236,72,153,0.8)]">Authorized Agents</h3>
                  </div>
                  <div className="max-h-60 overflow-y-auto">
                    {agents && agents.length > 0 ? (
                      agents.map(agent => (
                        <div key={agent.id} className="px-4 py-3 text-sm text-pink-100 border-b border-pink-500/20 last:border-0 flex items-center gap-2 hover:bg-[#2a2a4a] transition-colors">
                          <div className="w-2 h-2 rounded-full bg-pink-500 drop-shadow-[0_0_3px_rgba(236,72,153,0.8)]"></div>
                          {agent.name}
                        </div>
                      ))
                    ) : (
                      <div className="px-4 py-4 text-xs text-pink-500/70 italic text-center">
                        No agents assigned
                      </div>
                    )}
                  </div>
          ...
          <div className="relative w-full sm:w-auto">
            <button 
              ...
              className="bg-[#111] hover:bg-[#222] p-1.5 sm:p-3 rounded-lg sm:rounded-full border border-pink-500 text-cyan-400 shadow-[0_0_8px_rgba(236,72,153,0.5)] hover:shadow-[0_0_12px_rgba(236,72,153,0.8)] transition-all flex flex-row items-center justify-center gap-1 sm:gap-0 h-10 sm:h-auto w-full"
            >
            ...
            {showProfileMenu && (
              <>
                ...
                <div className="absolute top-full right-0 sm:-right-4 mt-2 w-48 bg-[#111] border border-pink-500 rounded-lg shadow-[0_0_15px_rgba(236,72,153,0.5)] overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                  <a 
                    href="/admin" 
                    target="_blank" 
                    rel="noopener noreferrer"
                    onClick={() => setShowProfileMenu(false)}
                    className="block px-4 py-3 text-sm text-[#f0ecd8] hover:bg-[#222] hover:text-cyan-400 font-bold transition-colors border-b border-pink-500/50"
                  >
                    Login as an Admin
                  </a>
                  <a 
                    ...
                    rel="noopener noreferrer"
                    onClick={() => setShowProfileMenu(false)}
                    className="block px-4 py-3 text-sm text-[#f0ecd8] hover:bg-[#222] hover:text-cyan-400 font-bold transition-colors"
                  >
                    Login as an Agent
                  </a>
                </div>
        ...
        {isLive && (
          <div className="space-y-5">
            ...
            <div className="flex flex-col items-center gap-2 py-3">
              <div className="flex items-center gap-2 bg-[#111] border border-pink-500/40 rounded-full px-5 py-2 shadow-[0_0_15px_rgba(236,72,153,0.3)]">
                {/* Status dot */}
                {gameStatus === 'running' ? (
                ...
                <span className="text-sm sm:text-base font-black text-cyan-400 tracking-[0.2em] uppercase">
            ...
            {latestWinner && (
              <div className="w-full max-w-lg mx-auto flex items-center gap-3 bg-pink-500/15 border border-pink-500/50 rounded-2xl px-4 py-3 animate-pulse shadow-[0_0_15px_rgba(236,72,153,0.3)]">
                <span className="text-2xl">🏆</span>
                <div>
                  <p className="text-pink-500 font-black text-sm tracking-wide uppercase">Winner!</p>
            ...
            {/*
            winners.length > 0 && (
              <div className="w-full max-w-lg mx-auto bg-[#111]/80 rounded-xl border border-pink-500/30 p-3 shadow-[0_0_10px_rgba(236,72,153,0.2)]">
                <p className="text-pink-500 text-[10px] font-bold uppercase tracking-widest mb-2">🏆 Winners ({winners.length})</p>
                  ...
                  {winners.map((w, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between text-xs">
                        <span className="text-cyan-400 font-medium">Ticket No. {tNo}{tName}</span>
                        <span className="text-purple-400">{w.matched_numbers?.length} numbers matched</span>
                      </div>
                    );
                  })}
                </div>
            ...
            */}
            ...
            <div className="flex flex-col items-center justify-center py-6 rounded-2xl bg-[#111] border border-pink-500 shadow-[0_0_20px_rgba(236,72,153,0.15)] relative overflow-hidden"
              style={{ minHeight: '160px' }}
            >
              {/* subtle bg pattern */}
              <div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at center, #ec4899 1px, transparent 1px)', backgroundSize: '16px 16px' }}></div>

              {latestNumber ? (
                <div className="relative z-10 flex flex-col items-center gap-1">
                  <p className="text-[10px] sm:text-xs font-bold text-cyan-400 uppercase tracking-widest mb-2">Number Called</p>
                ...
                <div className="flex flex-col items-center gap-2 opacity-50 relative z-10">
                  <div className="w-24 h-24 rounded-full border-4 border-dashed border-pink-500/50 flex items-center justify-center">
                    <span className="text-pink-500 text-3xl font-black">?</span>
                  </div>
                  <p className="text-xs text-purple-400 font-medium">Waiting for first number…</p>
                </div>
              )}
            </div>

            {/* ── 1–90 Number Grid ──────────────────────────────────────── */}
            <div className="rounded-xl bg-[#111] p-4 shadow-[0_0_15px_rgba(168,85,247,0.2)] border border-purple-500">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-xs font-black text-pink-500 uppercase tracking-widest">Number Board</h3>
                <div className="flex items-center gap-3 text-[10px] font-semibold text-cyan-400">
                  <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-pink-500 inline-block"></span>Called</span>
                  <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-[#222] border border-purple-500/50 inline-block"></span>Not yet</span>
                </div>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={[
                        ...
                        isLatest
                          ? 'bg-pink-500 text-white shadow-[0_0_15px_rgba(236,72,153,0.8)] scale-110 z-10 relative number-pop'
                          : isCalled
                          ? 'bg-purple-900 text-pink-300 shadow-sm border border-pink-500'
            ...
            <div className="mt-8 mb-4">
              <h3 className="text-sm font-black text-cyan-400 uppercase tracking-widest text-center mb-4 border-b border-purple-500 pb-2 drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]">Prize List</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {sortDividends(liveDividends.filter(d => d.is_active)).map((prize, idx) => {
                  ...
                  return (
                    <div key={prize.id || idx} className="bg-[#111]/80 border border-pink-500/50 rounded-xl p-3 flex flex-col shadow-[0_0_10px_rgba(236,72,153,0.1)]">
                      <div className="flex justify-between items-center border-b border-purple-500/50 pb-2 mb-2">
                        <span className="text-pink-400 font-bold text-xs sm:text-sm uppercase">{prize.name}</span>
                        <span className="text-cyan-400 font-black text-xs drop-shadow-sm">₹{prize.prize_amount?.toLocaleString('en-IN')}</span>
                      </div>
                      <div className="flex-1">
                        {prizeWinners.length > 0 ? (
                          <div className="space-y-1">
                            {prizeWinners.map((w, i) => (
                              <div key={i} className="flex items-center gap-2 bg-[#222] p-2 rounded-lg border border-purple-500/30">
                                <span className="text-lg drop-shadow-[0_0_5px_rgba(234,179,8,0.5)]">🏆</span>
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-cyan-400 text-[10px]">{w.matched_numbers?.length} matched</span>
                                </div>
                              </div>
                            ))}
                          </div>
                          ...
                          <div className="flex items-center justify-center h-full py-4 opacity-50">
                            <span className="text-purple-400 text-xs font-medium">Waiting for winner...</span>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {!isLive && (
          <>
            ...
            <button
              ...
              className="w-full flex items-center justify-center mb-4 py-3.5 sm:py-4 rounded-xl bg-gradient-to-r from-purple-900 via-purple-800 to-purple-900 border-2 border-pink-500 text-cyan-300 shadow-[0_0_25px_rgba(236,72,153,0.5)] hover:shadow-[0_0_35px_rgba(236,72,153,0.7)] transition-all active:scale-[0.98]"
            >
              <span className="font-black text-sm sm:text-base tracking-widest uppercase drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]">CHECK AVAILABLE TICKETS</span>
            </button>

        {/* STATS BAR */}
        <div className="flex flex-col rounded-xl bg-black p-3 shadow-[0_0_15px_rgba(236,72,153,0.2)] border border-pink-500 divide-y-2 divide-purple-500/20">
          {/* Row 1: Timer */}
          <div className="py-1 border-purple-500/20">
            <CountdownTimer
              targetDate={displayGame.scheduled_at || new Date().toISOString()}
              variant="split"
              className="w-full text-cyan-400 border-purple-500/20"
              numberClassName="text-xl sm:text-2xl font-black tracking-tight drop-shadow-[0_0_8px_rgba(34,211,238,0.5)]"
          ...
          <div className="grid grid-cols-2 pt-3 pb-1">
            <div className="flex flex-col items-center justify-center border-r-2 border-purple-500/20 px-2 text-center">
              <span className="text-sm sm:text-base font-bold text-cyan-400">{formattedDate}</span>
            </div>
            <div className="flex flex-col items-center justify-center px-2 text-center">
              <span className="text-sm sm:text-base font-black text-cyan-400">{formattedTime}</span>
            </div>
          </div>
          
          {/* Tickets Bought (Commented out as requested) 
          <div className="flex flex-col items-center justify-center pt-3 border-t-2 border-purple-500/20 text-center">
            <span className="text-[11px] sm:text-xs font-bold text-cyan-400 flex items-center justify-center gap-1"><span className="text-xl">🎟</span> Tickets Bought</span>
        ...
        <div className="rounded-xl bg-[#eef0e5] shadow-lg relative p-4 border-2 border-purple-500">
          <div className="flex justify-center mb-5 relative">
            <div className="bg-purple-900 text-pink-300 px-8 py-1.5 rounded-full font-bold tracking-widest text-sm flex items-center gap-2 shadow-md">
              <span className="text-green-400">🌿</span> PRIZE LIST <span className="text-green-400">🌿</span>
            </div>
          </div>
          
          <div className="flex flex-row items-center gap-4">
            <div className="w-24 h-32 sm:w-40 sm:h-40 shrink-0 relative flex items-center justify-center bg-gradient-to-b from-[#222] to-transparent rounded-lg border-2 border-purple-500">
              ...
              {sortDividends(liveDividends.filter(d => d.is_active)).map((prize, index) => {
                ...
                <div key={prize.id || index} className="flex items-center justify-between border-b border-[#c2bda2] pb-1.5 last:border-0 last:pb-0">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full ${colors[index%5] || 'bg-purple-900 text-pink-300'} flex items-center justify-center text-xs font-bold shadow-md border border-white`}>
                      {index + 1}
                    </div>
                    <span className="text-cyan-400 text-xs sm:text-sm font-semibold">{prize.name}</span>
                  </div>
                  <span className="text-cyan-400 font-black text-sm sm:text-base">₹{prize.prize_amount?.toLocaleString('en-IN')}</span>
                </div>
              )})}
        ...
        */}
        ...
        {/* TICKETS SECTION */}
        <div className="pt-2 border-2 border-purple-500 bg-black rounded-xl p-3 shadow-inner mt-4">
          {/* Banner */}
          ...
          <div className="flex w-full gap-1 mb-4">
            <button 
              ...
              className={`flex-1 font-bold py-2 rounded-t text-[9px] sm:text-xs tracking-tight border-t-2 border-x-2 border-b-2 border-purple-500 ${filter === 'all' ? 'bg-purple-900 text-pink-300 shadow-md' : 'bg-black text-purple-400'}`}
            >
              ALL TICKETS ({totalCount})
            </button>
            <button 
              onClick={() => { setFilter('booked'); setCurrentPage(1); }}
              className={`flex-1 font-bold py-2 rounded-t text-[9px] sm:text-xs tracking-tight border-t-2 border-x-2 border-b-2 border-purple-500 ${filter === 'booked' ? 'bg-purple-900 text-pink-300 shadow-md' : 'bg-black text-purple-400'}`}
            >
              TICKETS SOLD ({bookedCount})
            </button>
            <button 
              onClick={() => { setFilter('available'); setCurrentPage(1); }}
              className={`flex-1 font-bold py-2 rounded-t text-[9px] sm:text-xs tracking-tight border-t-2 border-x-2 border-b-2 border-purple-500 ${filter === 'available' ? 'bg-purple-900 text-pink-300 shadow-md' : 'bg-black text-purple-400'}`}
            >
...
function RealTicketCard({ 
  ...
  return (
    <div className={`relative rounded-xl overflow-hidden border-2 shadow-lg transition-all ${isSelected ? 'border-pink-500 bg-pink-900/30' : 'border-purple-500 bg-[#111]'} ${isBooked && !isLive ? 'opacity-90' : ''} ${isRetired ? 'opacity-60 grayscale' : ''}`}>
      {isRetired && (
      ...
      <div className={`flex justify-between items-center px-4 py-2 border-b-2 transition-colors ${isSelected ? 'border-purple-500 bg-purple-900/30' : 'border-purple-500 bg-[#111]'}`}>
        <div className="flex items-center gap-1.5 truncate pr-2 w-full">
          <span className="text-cyan-400 font-black text-xs sm:text-sm tracking-wide shrink-0">Ticket No. {ticket.ticket_number}</span>
          {isBooked && ticket.player_name && (
            <span className="text-cyan-400 font-bold text-[10px] sm:text-xs truncate w-full">
              • {ticket.player_name}
            </span>
          )}
          {!isBooked && (
              ...
              <>
                {onToggleSelect ? (
                  <button 
                    ...
                    className={`text-[9px] font-bold px-3 py-1 rounded shadow transition-colors border-2 ${isSelected ? 'bg-yellow-500 text-cyan-400 border-yellow-500' : 'bg-[#d97706] hover:bg-amber-700 text-white border-transparent'}`}
                  >
      ...
      <div className="p-0">
        <div className={`overflow-hidden transition-colors`}>
          {(ticket.grid || []).map((row, i) => (
            <div key={i} className={`flex w-full border-b-2 last:border-0 transition-colors ${isSelected ? 'border-purple-500' : 'border-purple-500'}`}>
              {row.map((num, j) => {
